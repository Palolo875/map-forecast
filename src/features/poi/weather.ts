...
export async function fetchCurrentWeatherSnapshot(lat: number, lng: number): Promise<CurrentWeatherSnapshot> {
  const url = `${OPEN_METEO_BASE_URL}/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,weather_code,wind_speed_10m,wind_direction_10m,precipitation,cloud_cover&timezone=auto`;
  ...
  return {
    ...
    windDirectionDeg: typeof c?.wind_direction_10m === "number" ? Math.round(c.wind_direction_10m) : undefined,
